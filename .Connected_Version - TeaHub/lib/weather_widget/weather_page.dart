import 'package:TeaHub/weather_widget/weather_model.dart';
import 'package:TeaHub/weather_widget/weather_service.dart';
...
class _WeatherDisplayState extends State<WeatherDisplay> {
  ...
  String getWeatherAnimation(String? mainCondition) {
    ...
    if (mainCondition == null) {
      return 'weather_animation/loading.json'; // Default weather condition
    }

    switch (mainCondition.toLowerCase()) {
      case 'clear':
        if (isHourInRange(now) && mainCondition.toLowerCase() == 'clear') {
          return 'weather_animation/clear_night.json';
        } else {
          return 'weather_animation/clear_day.json';
        }

      case 'clouds':
        if (isHourInRange(now) && mainCondition.toLowerCase() == 'clouds') {
          return 'weather_animation/cloudy_night.json';
        } else {
          return 'weather_animation/cloudy_day.json';
        }
      ...
      case 'dust':
        if (isHourInRange(now) && mainCondition.toLowerCase() == 'mist') {
          return 'weather_animation/mist_night.json';
        } else {
          return 'weather_animation/mist_day.json';
        }

      case 'rain':
        if (isHourInRange(now) && mainCondition.toLowerCase() == 'rain') {
          return 'weather_animation/rain_night.json';
        } else {
          return 'weather_animation/rain_day.json';
        }

      case 'drizzle':
      case 'shower rain':
        if (isHourInRange(now) && mainCondition.toLowerCase() == 'drizzle') {
          return 'weather_animation/partly_rain_night.json';
        } else {
          return 'weather_animation/partly_rain_day.json';
        }

      case 'thunderstorm':
        if (isHourInRange(now) &&
            mainCondition.toLowerCase() == 'thunderstorm') {
          return 'weather_animation/thunder_strom_night.json';
        } else {
          return 'weather_animation/thunder_strom_day.json';
        }
  ...
  bool isHourInRange(DateTime dateTime) {
    ...
    return (hour >= 18 && hour <= 24) || (hour >= 0 && hour <= 4);
  }
